...
$sql = "SELECT prod.preco, prod.id_curso, prod.nome FROM cursos prod WHERE id_curso = '" . $Dados['reference'] . "'";
...
if ($xml->paymentMethod->type == 1) {
    $result_cadastrar = "INSERT INTO pagamento (tipo_pg, cod_trans, status, referencia, created) VALUES (:tipo_pg, :cod_trans, :status, :referencia, NOW())";
    $cadastrar = $conn->prepare($result_cadastrar);
} elseif ($xml->paymentMethod->type == 2) { //QUERY para pagamento com boleto
    $result_cadastrar = "INSERT INTO pagamento (tipo_pg, cod_trans, status, link_boleto, referencia, created) VALUES (:tipo_pg, :cod_trans, :status, :link_boleto, :referencia, NOW())";
    $cadastrar = $conn->prepare($result_cadastrar);
    ...
    $result_cadastrar = "INSERT INTO pagamento (tipo_pg, cod_trans, status, link_db_online, referencia, created) VALUES (:tipo_pg, :cod_trans, :status, :link_db_online, :referencia, NOW())";
    $cadastrar = $conn->prepare($result_cadastrar);
